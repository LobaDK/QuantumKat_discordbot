...
class Activity(commands.Cog):
    def __init__(self, bot):
        ...
        if 'discord.Activity' in logging.Logger.manager.loggerDict:
            ...
            handler = logging.FileHandler(filename='logs/activity.log', encoding='utf-8', mode='a')
        ...
        self.messages = ['{hiss} in the {ordinal} {location}',
                         ...
                         ('Segfault in... ErrorIt?656E64\t\trebooting faulty'
                          '... rF7_Q>~bTV'),
...
# command splitter for easier reading and navigating
    ...
    @commands.command(aliases=['activitystop', 'astop'],
                      brief='(Bot owner only) Stops the displayed activity.',
                      description=('Stops the loop that displays a random '
                                   'activity under the bot.'))
    @commands.is_owner()
...
# command splitter for easier reading and navigating
    ...
    @commands.command(aliases=['activityrestart',
                               'arestart',
                               'ActivityRefresh',
                               'activityrefresh',
                               'arefresh'],
                      brief=('(Bot owner only) Restarts/Refreshes the '
                             'displayed activity.'),
                      description=('Restarts/Refreshes the loop that displays '
...
# command splitter for easier reading and navigating
    ...
    @commands.command(aliases=['activitystart',
                               'astart'],
                      brief=('(Bot owner only) Starts displaying a random '
                             'activity.'),
                      description=('Starts the loop that displays a random '
