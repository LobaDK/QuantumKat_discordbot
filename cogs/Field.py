...
class Fields(commands.Cog):
    ...
    async def searchfiles(self, search_keyword: str) -> list[str]:
        ...
        p = await create_subprocess_shell(
            f'find {self.a_folder} -maxdepth 1 -type f -iname "*{search_keyword}*"',
    ...
    @commands.command(
        aliases=["arr", "arrr", "arrrr", "arrrrr"],
        brief="Returns a random file from aaaa.lobadk.com.",
        description="Takes no arguments, but up to 5 r' can be appended, each fetching another random file from aaaa.lobadk.com.",
    )
    ...
    @commands.command(
        aliases=["or", "orr", "orrr", "orrrr", "orrrrr"],
        brief="Returns a random file from possum.lobadk.com.",
        description="Takes no arguments, but up to 5 r' can be appended, each fetching another random file from possum.lobadk.com.",
    )
    ...
    async def pet(self, ctx: commands.Context, *, optional_user_or_object: str = ""):
        ...
        if not optional_user_or_object:
            await ctx.send(
                f"Superpositions {quantum_amount} {verb} around {ctx.author.mention} and {pets}"
            )
            return
        ...
        if mention in optional_user_or_object:
            ...
            async def quantumloop(self, ctx):
                petloop = "".join(
                    choices(["pet", "pat", "petting", "patting"], k=randint(8, 40))
                )
                await ctx.send(
                    f"Quantum Loop pet initiated trying to pet self! {petloop}"
                )

            await choices(
                [quantumpurr, quantumfrequency, quantumloop], k=1, weights=[10, 10, 2]
            )[0](self, ctx)

        else:
            await ctx.send(
                f"Superpositions {quantum_amount} {verb} around {optional_user_or_object} and {pets}",
                silent=True,
            )
    ...
    async def hug(self, ctx: commands.Context, *, optional_user_or_object: str = ""):
        ...
        if not optional_user_or_object:
            await ctx.send(
                f"Superpositions {quantum_amount} {verb} around {ctx.author.mention} and {hugs}"
            )
            return
        ...
        if mention in optional_user_or_object:
            ...
            await ctx.send(
                f"Superpositions {quantum_amount} {verb} around {optional_user_or_object} and {hugs}",
                silent=True,
            )
    ...
    async def quantumpet(
        ...
        if not optional_user_or_object:
            await ctx.send(
                f"Superpositions {quantum_amount} times across all timelines, dimensions, universes and realities, and {qpets} {ctx.author.mention}"
            )
            ...
            await ctx.send(
                f"Superpositions {quantum_amount} times across all timelines, dimensions, universes and realities, and {qpets} {optional_user_or_object}",
                silent=True,
            )
    ...
    async def quantumhug(
        ...
        if not optional_user_or_object:
            await ctx.send(
                f"Superpositions {quantum_amount} times across all timelines, dimensions, universes and realities, and {qhugs} {ctx.author.mention}"
            )
        elif mention in optional_user_or_object:
            quantumhugloop = "".join(
                choices(
            ...
            await ctx.send(
                f"Superpositions {quantum_amount} times across all timelines, dimensions, universes and realities, and {qhugs} {optional_user_or_object}",
                silent=True,
            )
    ...
    async def rps(self, ctx: commands.Context, *, item: str = ""):
        ...
        if any(_ in item for _ in substring):
            ...
            if item == "scissor":
                ...
                item = item[:-1]
            if whowin == "I win!":
                winItem = {"rock": "paper", "paper": "scissors", "scissors": "rock"}

                await ctx.reply(
                    f"{winItem[item]}. {whowin} You do know I'm a quantum kat, right?",
                    silent=True,
                )
            else:
                winItem = {"paper": "rock", "scissors": "paper", "rock": "scissors"}
            ...
            if allowed.match(search_keyword):
                ...
                if len(files) == 0:
                    ...
                    if len(" ".join(files)) >= 4000:
                        await ctx.reply(
                            "Too many results! Try narrowing down the search",
                            silent=True,
                        )
                    else:
                ...
                await ctx.reply(
                    "At least two alphanumeric characters are required, or 1 alphanumeric character and a `.` at the start or end",
                    silent=True,
                )
        else:
            await ctx.reply(
                "Search too short! A minimum of 2 characters are required", silent=True
            )
            ...
            if not allowed.match(filename):
                ...
                if len(links) == 1:
                    ...
                    if len(links) == 0:
                        ...
                        await ctx.reply(
                            f"Found {len(links)} files. Please be more precise!"
                        )
        else:
            await ctx.reply(
                "At least a partial filename is required!\n```?a example```",
                silent=True,
            )
    ...
    @commands.command(
        aliases=["qjoke", "joke", "qj"],
        brief="Fetch a random quantum joke.",
        description="Fetches a random quantum joke stored in a text file. Supports no arguments.",
    )
    ...
    @commands.command(
        aliases=["ars", "asr", "aaaarandomsearch"],
    ...
    async def arsearch(self, ctx: commands.Context, search_keyword: str = ""):
        if search_keyword:
            ...
            if not allowed.match(search_keyword):
                ...
                await ctx.reply(
                    "Invalid character found in search parameter!", silent=True
                )
        else:
            await ctx.reply(
    ...
    async def ping(self, ctx: commands.Context, ping_mode: str = ""):
        LatencyResponses = [
            ...
            "That was a nice nap.",
            "Do you realize how many times I could've been pet in that amount of time!",
        ...
        if ping_mode:
            if ping_mode.casefold() == "latency":
                ...
                if latency < 0:
                    ...
                    latency += offset
                    await ctx.reply(
                        f"A negative latency value was detected. Used pool.ntp.org to attempt to correct, with a {round(offset)}ms offset.",
                        silent=True,
                    )

                await ctx.reply(
                    f"{pingresponse} Took {latency * 2}ms. {choice(LatencyResponses)}\nConnection to discord: {round(self.bot.latency * 1000)}ms",
                    silent=True,
                )
            else:
    ...
    @commands.command(
        aliases=["acount", "countaaaa", "counta"],
        brief="Counts the images and videos stored on aaaa.lobadk.com.",
        description="Counts the amount of images and videos that are stored on aaaa.lobadk.com, excluding any unrelated files, as well as folders.",
    )
    async def aaaacount(self, ctx: commands.Context):
        all_files = await self.getfiles(self.aURL, self.a_folder)
        await ctx.reply(
            f"There are currently {len(all_files)} quantised datasets.", silent=True
        )

    ######################################################################################################

    @commands.command(
        aliases=["quantumball", "qball"],
        brief="standard 8ball replies, but with some quantum mixed.",
        description="Uses the standard 8ball replies, but with some quantum related replies or addition to the messages. Accepts no arguments.",
    )
    async def QuantumBall(self, ctx: commands.Context):
        affirmative_answers_list = [
            "It is certain.",
            "It is decidedly so.",
            "Without a doubt.",
            "Yes definitely.",
        ...
        reason_list = [
            "Quantum tunnel unexpectedly collapsed!",
            "Entanglement lost!",
            "Was that vase always broken?",
            "Error",
            "||redacted||",
            "Universal instability detected!",
        ]
        quantumball_messages_list = [
            "{amount} {location} agrees or already happened in. {affirmative_answer}",
            "{reason}. {non_committal_answer}",
            "No related parallel universe, dimension, timeline or reality detected! {negative_answer}",
        ]

        amount = randint(100, 100_000)

        await ctx.reply(
            choices(quantumball_messages_list, k=1, weights=[10, 5, 5])[0].format(
                amount=amount,
                location=choice(["realities", "universes", "dimensions", "timelines"]),
                affirmative_answer=choice(affirmative_answers_list),
                reason=choice(reason_list),
                non_committal_answer=choice(non_committal_answers_list),
