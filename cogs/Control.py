...
class Control(commands.Cog):
    ...
    @commands.command(aliases=['serverownerlist',
                               'SOL'],
                      brief=("(Bot owner only, limited to DM's) Fetches a "
                             "list of servers the bot is in."),
                      description=("Fetches a list of servers the bot is in, "
    ...
    async def ServerOwnerList(self, ctx):
        ...
        for guild in self.bot.guilds:
            servers_and_owners.append((f'Server: {guild.name} '
                                       f'with ID: {guild.id}, '
    ...
    @commands.command(aliases=['leaveserver'],
                      brief='(Bot owner only) Leaves a server.',
                      description=('Leaves the ID-specified server. ID can be '
    ...
    @commands.command(aliases=['leave'],
                      brief=('(Bot owner, server owner and admin/mods only) '
                             'Leave current server.'),
                      description=('Leaves the server the message was sent '
    ...
    @commands.command(aliases=['LP'],
                      brief=('(Bot owner only) Lists permissions given to the '
                             'bot.'),
